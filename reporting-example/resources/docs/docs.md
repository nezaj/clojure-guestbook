...
The `home-routes` handler in the `reporting-example.routes.home` namespace
...
The `home-page` function will in turn call the `reporting-example.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `reporting-example.handler` namespace:
...
Request middleware functions are located under the `reporting-example.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `reporting-example.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `reporting-example.db.core` in some other namespace.
